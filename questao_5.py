...
for _ in range (numero_candidatos):
    ...
    nota = float(nota)
    informacoes_candidato = [nome, nota, grupo_candidato]
    lista_candidatos.append([nome, nota, grupo_candidato])
    
candidatos_nao_selecionados = lista_candidatos.copy()

for i in range (len(lista_candidatos)):
    if lista_candidatos[i][2] !='G' and dicionario_grupos[lista_candidatos[i][2]] != 0:
        dicionario_grupos[lista_candidatos[i][2]] -= 1
